package mesos

import (
	"bytes"
	"strings"
	"testing"
)

const cfg = `
CPU = "100"
Memory = "100Gi"
Pods = "20"`

func TestConfig(t *testing.T) {
	br := bytes.NewReader([]byte(cfg))
	var p Provider
	err := p.loadConfig(br)
	if err != nil {
		t.Error(err)
	}

	wanted := "100"
	if p.config.CPU != wanted {
		t.Errorf("Wanted %s, got %s.", wanted, p.config.CPU)
	}

	wanted = "100Gi"
	if p.config.Memory != wanted {
		t.Errorf("Wanted %s, got %s.", wanted, p.config.Memory)
	}

	wanted = "20"
	if p.config.Pods != wanted {
		t.Errorf("Wanted %s, got %s.", wanted, p.config.Pods)
	}
}

const cfgBad = `
CPU = "-100"
Memory = "-100Gi"
Pods = "-20"`

func TestBadConfig(t *testing.T) {
	br := bytes.NewReader([]byte(cfgBad))
	var p Provider
	err := p.loadConfig(br)
	if err == nil {
		t.Fatal("expected loadConfig to fail with bad operating system option")
	}

	if !strings.Contains(err.Error(), "is not a valid operating system") {
		t.Fatalf("expected loadConfig to fail with 'is not a valid operating system' but got: %v", err)

	}
}

const defCfg = `
NonExistingOption = "xpto"
`

func TestDefaultedConfig(t *testing.T) {
	br := bytes.NewReader([]byte(defCfg))
	var p Provider
	err := p.loadConfig(br)
	if err != nil {
		t.Error(err)
	}
	// Test that defaults work with no settings in config.
	wanted := "20"
	if p.config.CPU != wanted {
		t.Errorf("Wanted default %s, got %s.", wanted, p.config.CPU)
	}

	wanted = "100Gi"
	if p.config.Memory != wanted {
		t.Errorf("Wanted default %s, got %s.", wanted, p.config.Memory)
	}

	wanted = "20"
	if p.config.Pods != wanted {
		t.Errorf("Wanted default %s, got %s.", wanted, p.config.Pods)
	}
}
